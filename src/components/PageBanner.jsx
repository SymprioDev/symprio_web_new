...
const PageBanner = ({ title, breadcrumb = [], backgroundImage }) => {
  ...
  return (
    <section className="relative min-h-[420px] md:min-h-[500px] w-full overflow-hidden flex items-center">
      {/* Background Image */}
      <div
      ...
      <div className="container mx-auto px-6 relative z-10 pt-20">
        <h1 className="max-w-5xl text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-semibold leading-tight text-white mb-6 break-words">
          {title}
        </h1>
      </div>
      ...
      <div className="absolute bottom-0 left-0 bg-white px-8 py-4 rounded-tr-3xl z-20 hidden md:block border-t border-r border-gray-100">
        <nav className="flex items-center space-x-2 text-sm font-medium">
          <Link to="/" className="text-gray-500 hover:text-[#185ADB] flex items-center">
          ...
          {breadcrumb.map((item, index) => (
            <React.Fragment key={index}>
              ...
              {item.link ? (
                <Link to={item.link} className="text-gray-500 hover:text-[#185ADB]">
                  {item.label}
                </Link>
              ) : (
                <span className="text-[#185ADB]">{item.label}</span>
              )}
            </React.Fragment>
          ))}
        </nav>
